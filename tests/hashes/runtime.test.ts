...
import { HashingContext } from '../../dist/types/driver';
...
describe('hasher: (function, runtime) generates a hashing implementation and ensures its integrity', function () {
  for (const _id of getHashes()) {
    it (`should properly hash with the '${_id}' algorithm (1)`, async function () {
      ...
      const driver = getHasher(_id)
      const digested = await driver.digest(ctx)
      expect(digested.content).to.not.equal(ctxBody)
    })
    it (`should properly hash with the '${_id}' algorithm (128)`, async function () {
      const ctx: HashingContext = {
        content: ctxBody,
        digest: 'base64',
        iter: 128
  ...
  it ('should default to base64 encoding when invalid encoding is provided', async function () {
    const ctxr = await getHasher('sha256').digest({
      content: 'hello world',
      // @ts-expect-error disable type checking for purpose of test environment
      digest: 'latin'
    })
    expect(ctxr.content).to.equal('uU0nuZNNPgilLlLX2n2r+sSE7+N6U4DukIj3rOLvzek=')
  })
})
