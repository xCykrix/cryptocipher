...
const { superify } = require('../../dist/lib/super/super.hashing')
...
describe('hasher: generate consistent integrity with hash events', function () {
  const disabled = superify().disabled
  for (const _id of getHashes()) {
    it(`executes algorithm '${_id}' [short]`, async function () {
      if (disabled.includes(_id)) return this.skip()
