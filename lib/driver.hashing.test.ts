...
import { superify } from './super/super.hashing';
...
describe(`Side-by-side Check - Test 2 - Stateful Integrity - '${location}'`, function () {
  ...
  it('should default to "base64" when the encoding is not acceptable or unsupported', async function () {
    const digestBase64 = await getHasher('sha1').digest({
      content: 'hello world',
      digest: 'base64',
    });
    const digestBase64Defaulted = await getHasher('sha1').digest({
      content: 'hello world',
      digest: 'base64url',
    });
